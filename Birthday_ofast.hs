-- ghc -O2 -threaded -o birthday Birthday_ofast.hs && ./birthday +RTS -N
...
simulate :: Int -> Word64 -> Int
simulate threadId seed = runST $ do
...
main = do
  ...
  successCount <- forM [0 .. numThreads - 1] $ \t -> do
    count <- newEmptyMVar
    forkIO $ putMVar count $! simulate t seed
    return count
