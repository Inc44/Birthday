...
let get_cpu_count () =
	try
		let input_channel = Unix.open_process_in "getconf _NPROCESSORS_ONLN" in
		let stdout = try input_line input_channel with _ -> "1" in
		ignore (Unix.close_process_in input_channel);
		let cpus = try int_of_string (String.trim stdout) with _ -> 1 in
		max 1 cpus
	with _ -> 1

let threads =
	let cpus = get_cpu_count () in
	min cpus num_threads

let simulations_per_thread = total_simulations / threads
...
let () =
	...
	let threads =
		Array.init threads (fun t ->
				Thread.create (fun id -> simulate id data_success_count) t)
	in
